...
pub mod macaroni {
    ...
    impl Macaroni {
        ...
        fn run_tokens(&self, tokens: Vec<Token>) {
            let (mut stack, mut return_vals) =
                (Vec::<&Token>::new(), Vec::<Token>::new());
            let mut idx = 0;
            while idx < tokens.len() {
                ...
                match t {
                    ...
                    _ => {
                        ...
                        let mut return_val: Option<Variable> = None;
                        'w: while let Some(pos) = stack.iter().rposition(|x|
                            ...
                            if let &Token::Op { ref func, ref arity } = stack[pos] {
                                if *arity == args.len() {
                                    return_val = func(&args.into_iter()
                                        .map(|t|
                                            match t {
                                                &Token::Var(ref v) => v,
                                                _ => panic!("???")
                            ...
                            //if return_val.is_some() {
                                return_vals.push(Token::Var(return_val.unwrap()));
                                stack.push(return_vals.get(return_vals.len() - 1).unwrap());
                            //}
                            if finished {
                                break 'w;
                            } else {
